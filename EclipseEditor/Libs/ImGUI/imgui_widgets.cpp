...
// Custom Slider with Image Grab
bool ImGui::ImageSlider(const char* label, ImTextureID user_texture_id, const ImVec2& image_size, const ImVec2& size, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
	if (!user_texture_id || image_size.x <= 0 || image_size.y <= 0)
	{
		// Fallback to default slider if texture is not valid
	...
	// --- 1. Make default grab invisible and set its size ---
	PushStyleColor(ImGuiCol_SliderGrab, ImVec4(0, 0, 0, 0));
	PushStyleColor(ImGuiCol_SliderGrabActive, ImVec4(0, 0, 0, 0));
	// Set the grab size to match the image height. This helps SliderBehavior's calculations.
	PushStyleVar(ImGuiStyleVar_GrabMinSize, (float)image_size.y);

	// --- 2. Draw the "invisible" slider ---
	// We use SliderScalar instead of SliderFloat for more control if needed,
	// but SliderFloat is fine. For simplicity here, just SliderFloat.
	bool value_changed = VSliderFloat(label, size, v, v_min, v_max, format, flags);
	...
	if (style.Alpha != 1.f)
		alpha = 0;
	ImColor image_color = { 255, 255, 255, alpha};
	if (IsItemActive())
		image_color = style.Colors[ImGuiCol_SliderGrabActive];

	// --- 4. Get slider's bounding box (of the last drawn item) ---
	ImRect slider_bb(ImGui::GetItemRectMin(), ImGui::GetItemRectMax());

	// --- 5. Calculate image position ---
	// The slider's active area for the grab starts after FramePadding.x
	// and its width is (total_width - 2 * FramePadding.x).
	// The grab itself (now sized by grab_texture.width due to PushStyleVar) slides within this.
	float T = 0.0f;
	if (v_max > v_min)
	{
	...
	// Determine the Y position of the left edge of the grab area
	// This calculation is similar to what happens inside ImGui::SliderBehavior
	// The grab slides within the range [slider_usable_x_min, slider_usable_x_max - grab_width]
	float slider_usable_h = slider_bb.GetHeight() - style.FramePadding.y * 2.0f;
	float grab_render_height = (float)image_size.y;

	// Calculate the left edge of where the logical grab (now sized by grab_texture.width) would be drawn.
	// ImLerp is (a + (b-a) * t)
	// The range the grab's left edge can be in is:
	// [slider_bb.Min.x + style.FramePadding.x,
	//  slider_bb.Max.x - style.FramePadding.x - grab_render_width]
	float grab_y_pos = ImLerp(
		slider_bb.Max.y - style.FramePadding.y - grab_render_height,
		slider_bb.Min.y + style.FramePadding.y,
		T
	);

	// Horizontally center the image on the slider
	float image_x_pos = slider_bb.GetCenter().x - (float)image_size.x / 2.0f;

	ImVec2 image_top_left(image_x_pos + image_size.x, grab_y_pos + image_size.y);
	ImVec2 image_bottom_right(image_x_pos, grab_y_pos);

	// --- 6. Draw the image ---
	// Ensure UVs are correct if your texture is part of an atlas
	ImVec2 uv0(0.0f, 0.0f); // Top-left UV
	ImVec2 uv1(1.0f, 1.0f); // Bottom-right UV
