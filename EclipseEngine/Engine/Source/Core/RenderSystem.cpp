...
namespace Core
{
	...
	void RenderSystem::Render(RHI::IRenderInterface* _renderInterface, RHI::IGraphicPipeline* _pipeline, Math::Mat4 _V, Math::Mat4 _P, Math::Vec3 _viewPos, std::vector<ParticleEmitterRenderData> _particlesData)
	{
		if (_renderInterface != nullptr && _pipeline != nullptr)
		{
			...
			_pipeline->Draw(_V, _P, _viewPos, GetStaticModels(), GetLightsData(), _particlesData);
		}
	}
	...
	RHI::LightsData RenderSystem::GetLightsData() const
	{
		std::vector<RHI::DirLightData> dirLightsData = GetDirLights();
		std::vector<RHI::PointLightData> pointLightsData = GetPointLights();
		std::vector<RHI::SpotLightData> spotLightsData = GetSpotLights();

		int dirLightsNb = static_cast<int>(dirLightsData.size());
		int pointLightsNb = static_cast<int>(pointLightsData.size());
		int spotLightsNb = static_cast<int>(spotLightsData.size());

		return RHI::LightsData(
			m_ambientLight / 255.f,
			dirLightsData,
			pointLightsData,
			spotLightsData,
			dirLightsNb,
